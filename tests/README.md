...
**`run-tests.js`** — the backend on its own: exact cutoff-note text (both the `Remaining` and the `Short` branch), cutoff maths and the accounting identity, salary snapshotting and the blank-cell fallback, the entered Split and its default, the computed stock ledger (baseline, deliveries, stocktakes, the reorder warning), the Maintenance writers, the DailyCounts grid-capacity boundary (the case that could once wipe per-item history), calendar-date validation, Cutoffs upsert, idempotent replays, price snapshots, the backlog seeds, and — v2.8.0 — the `costing` arithmetic against hand-computed figures, with the two fences that make it safe (a restock counted once, and — since v2.22.0 — a cost moving the residual by exactly the value opened and nothing else, with a cost edited *after* a night was saved moving nothing at all).
...
| server: the cash-floor refusal removed | 2 — the cash floor: converted cash above the day's cash is refused naming both figures *(renamed in v2.25.0: "…above the cutoff's cash so far…")* |
...
### v2.22.0 — the opened value is an allocation

Four new tests, **22 mutants, all red** — but the work was **thirteen existing fences that asserted the opposite of the new rule**, and how to turn them around without gutting them. And one bug found by *reading*, not by a failure, that the tests then had to be made capable of catching.

**The rounding bug, and the test that did not catch it.** Code.gs rounds the opened value and *then* subtracts it; the phone was subtracting the raw sum and rounding once at the end. A unit cost with more than two decimals puts the two a centavo apart. I wrote a test with `33.333 × 3 = 99.999` to prove it — and the buggy code **passed**, because `.001` rounds away. The fixture was rebuilt with `120.375 × 3 = 361.125` (exact in binary, so it cannot flake), where subtracting raw leaves `.875 → .88` and subtracting rounded leaves `.13`: two *different* numbers. The lesson is general — **a fixture must make the wrong answer a different number, not a number that rounds to the same one.**

**The seam had a hole.** `SHARED_FIGURES` — the list of figures the phone and the sheet must agree on — never included `remaining`. It was covered only through the note text, and `fmt()` rounds a centavo of drift into agreement. `remaining`, `salary`, `split` are in it now, and a mapped list compares the figures the two sides spell differently (`supplies_used` / `suppliesUsed` …), because a seam that only checks same-spelled keys quietly skips exactly the ones added most recently.

`THE FENCE: the cutoff note and its figures are byte-identical with and without costs` has now moved three times: v2.8.0 said a cost changes nothing; v2.15.0 said nothing but a footer; v2.22.0 says a cost moves the **residual** and nothing else — by **exactly** the value opened. Each time, the temptation was to delete it. Each time, what survived is *stronger*: "nothing moved" is one bit; "moved by precisely this amount, to the centavo, in this direction" pins the arithmetic both ways and catches a double-deduction, a sign flip and rounding creep that the original could not.

`stock never reaches the cutoff figures or the note` was **renamed** to *"stock QUANTITIES never reach the cutoff figures — only their COST does"*, because the real danger it guarded was always a *quantity* leaking into a money figure. It now asserts `remaining !== -2000 - 5` explicitly: 5 packs must never be spent as if they were ₱5.

`SPEC_NOTE_USED` now carries the residual shift (`Short - 4,940`) rather than each call site patching it in, so the fixture and the rule cannot drift. `noteFixed()` strips the two lines a cost may touch; `noteSums()` stays for the fences that still want only the one.

**One survivor was fixture blindness, again.** `backlog_in_minor` summing *every* category survived because the period had no other expense — 900 either way. A Supplies row alongside makes the wrong answer 1,150.

**The 99,999-pack test is a good one.** It piles absurd usage on and asserts the residual moves by `after.suppliesUsed - before.suppliesUsed` exactly. The resulting figure is −12,004,580, and it is the *right* monstrous figure — an assertion that merely forbade movement would have hidden whether the arithmetic was correct at all.

**Then the adversarial review found what the tests had made true.** Every fence above asserted *"a cost moves the residual by exactly the value opened"* — and that was precisely the defect: a cost edited under Maintenance **after** a fortnight was generated moved that fortnight's Remaining, and Generate overwrote the archive. The tests were faithfully pinning a restatement. Three more tests and the fence staged in two: **Stage 1** blanks only the current costs (what a Maintenance edit does) and asserts the saved period does not move by a centavo and the note is byte-identical; **Stage 2** blanks the row snapshots too — the sheet as it stood before anyone priced anything — and only then makes the with-costs / no-costs comparison the fence always made. Two of the existing v2.22.0 tests had to be restaged the same way, because they too were built on "blank the cost, expect zero". **19 mutants on the snapshot, all red** — the one first-pass survivor was the unpriced count summing rows rather than products, which a single unpriced night cannot distinguish; a second night of the same product makes the wrong answer 2.
...
Two tests — one behaviour pin, one source pin — and **10 mutants, all red** on the first pass.

**The rule is a function, so the mutants have to change an answer.** *"Auto place the value equal to the remaining"* was not implemented as *remaining* written into a field: since v2.23.0 a payment moves no figure above, so that card would offer the same ₱5,193 again the moment he had paid it. `backlogPayable(figures, balance)` is Remaining less what this cutoff has already paid down, capped at the chosen debt's balance, and it lives beside the other figure readers (the slab rule: beside its subject, the figures object — not beside the card that calls it). The behaviour pin walks it: nothing paid offers all of it; 5,000 paid offers 193; fully paid, overpaid, a short cutoff and a zero all offer **blank** — never `0` typed into a money field, never a negative; capped at a chosen ₱700 debt, uncapped when nothing is chosen, and **not capped to 0 by a debt with nothing owed**, which is the mutant a naive `Math.min` would let through; both casings of the paid figure, because the card is handed the phone's own camelCase figures; two decimals.

**The source pin holds what Node cannot drive.** The card computes the pre-fill from the *live* figures capped at the chosen debt; writes it into `payBl.amount` **only while untouched**, so "Log this payment" saves exactly what the field shows; says *"Change it if you are paying less"*; and no longer promises the payment *"shows on the note under 'Other payments'"* — a sentence that had been false since v2.23.0 and was still on the card. Typing sets `touched`; choosing a debt re-renders so the cap follows the choice; the initial state and the post-payment reset are both untouched, so the next pre-fill is what is *still* left.

**Proven in the preview, five steps on the September fixture** (Remaining 4,515, debts Flour 4,578 and Mayo 135): untouched, the field reads 4,515 with the pre-fill hint; Mayo chosen, 135 and *"capped at what is owed on Mayo"*; Flour chosen, back to 4,515; a typed 1,000 survives a full re-render with the hint gone; the payment logged, Remaining is still 4,515 (a payment moves no figure) and the field offers 3,515.

Suites: **210** (`run-tests.js`) + **282** (`contract.test.js`) + **12 phone screens** (`smoke/phone.js`).

### v2.24.0 — a cheese box paid by GCash is worth its whole price

Sixteen new tests, one turned, two new slabs — and an adversarial review that found two money bugs the tests had just pinned as correct.

**The state machines had never been lifted.** `uiStep` and `uiTyped` — the two functions that turn a press or a typed figure on the cheese/GCash questions into the four buckets — sat in the gap after `syncRowInputs` (where `S_BLANKS` ends) and before `BUCKETS`, so no slab lifted them and no test ever drove them. The presentation map lived right there: *"of those"* meant `G = gcash + gcashCheese`, and a press on it moved one box from `gcash` to `gcashCheese`, so GCash rose by the ₱15 add-on. Seven releases, 480-odd green tests, and the one function that decided how a GCash cheese box was priced on the nightly screen was outside every marker. They are `S_UISTEP` now, with `S_STEPPER` for the stepper markup, driven through `loadSyncClient` with `syncRowInputs`/`afterCountChange` recorded and every toast captured.

**The two nights, replayed press by press.** Sept 5 Box 6 (start 20, end 12; cheese +1, regular GCash +2, cheese GCash +1) lands as `{cheese 0, gcash 2, gcashCheese 1}`: amount 535 — the Total the sheet already holds for the row — and GCash **210**. The REAL server prices the same payload to the same peso. The old map's answer for those presses is what the sheet holds today, 145, and the test pins `210 − 145 = the row's regular price`. Sept 6 Box 10 likewise: 2,620 and **670** against the stored 460. Then each press on its own: *Cheese boxes* moves a whole box (GCash +80, Cash −80, Total unchanged), refuses at cheese 0 with the sentence that names *How many were cheese*, and comes back as cash cheese on −1; *Regular boxes* −1 at 0 does nothing (the old card reclassified here); *How many were cheese* −1 takes the cash cheese first and then the GCash one stays GCash as a regular box; every room includes the special order's boxes and the given-away units, with the server's own arithmetic in the toast. Typed figures: regular GCash moves nothing else (the old `uiTyped` rewrote `gcash` from `G − GC`), cheese by GCash is capped at the cheese counted, and a cleared box is 0 to the buckets.

**Nothing stored changes meaning.** A night saved by the old card loads back showing the sheet's buckets (C 1, G 1, GC 1), re-saves them untouched, and prices as the sheet did — 145 — until one press makes it 210 with the Total still 535. The paper-review test turned: `gcashAll` 4 → 3 (the plain boxes; the cheese one is counted apart), `uiVal G` 3, `GC` 1.

**The give-away card.** Its HTML rendered for real: every sku including nori, the same `data-field="free"` as before, closed and `hidden` when empty with a blank head, *"1 unit"* / *"3 units"* when held, one error slot per sku. Its load rule: an ordinary night closed, a night with two given away open, `freeQty` still on the wire and 5 sold less 2 unpaid still 3 × ₱50. Source pins on the wiring: the order (Box counts, Given away, Sold with GCash, Sold with cash), the `COLLAPSE_FLAG` whitelist, the `toggle-free` handler, `saveBenta` opening it before the scroll on a `free:` error, `updateReceipt` refreshing its head, `freeOpen` remembered, and `freeHeld` read in the same three places `gcashHeld` is. One of these tests was wrong first: it loaded a night that opens the card and then expected an ordinary night to load closed — but a card remembers its open state from one load to the next, exactly as the GCash card does. The ordinary night is loaded first now.

**Browser, at 375px, on the real screen.** The Sept 5 presses: receipt lines *Box 6 GCash regular ×2 130* and *Box 6 GCash cheese ×1 80*, GCash ₱210 / Cash ₱325 / Total ₱535, the fourth *Cheese boxes* press refused with its toast and nothing moved; the give-away card between Box counts and Sold with GCash, *"1 unit"* in its head, *"less 1 given away or ruined"* on the receipt; document 375px wide on a 375px screen. `smoke/phone.js` PASS on all 12 screens locally.

**The review, and what it taught the tests.** The owner asked for everything to be double-checked, *"specially the computation"*, so the change went through six independent reviewers (state machine, round trips, other consumers of the buckets, the give-away card, the data inference, the words), each finding then handed to two adversarial verifiers who had to reproduce it against the real functions or refute it. Twelve of the forty-two agents fell to a session limit — every mutation agent among them, so the mutants below were run by hand — but the review itself completed. **Two money findings, both confirmed twice.** *(a)* The press path and the typed path on *How many were cheese* disagreed: with every cheese box already GCash, a press of − moved the last one to **regular GCash** — bumping *Regular boxes*, his own count, and adding one regular price to GCash — while typing the same lower figure left it cash regular. Same visible answer, ₱65 apart. The test at *"the last cheese was GCash: it stays GCash, just not cheese"* had **pinned the wrong behaviour as right**: it was written from the old card's reasoning, where that move kept "Sold with GCash" constant. It now asserts the box is simply no longer cheese and *Regular boxes* is untouched, and a new test drives both paths to identical buckets, identical GCash and identical Total, on the Sept 5 shape too. *(b)* `stashBentaDraft` had never written `free` — since v2.10.1, five releases — while both readers accepted it; the new card turned a silent gap into a card that emptied itself on a date change. One key in the draft, and a test that types two give-aways, stashes, leaves, returns: 2 units, card open, `freeQty 2`, nobody charged — and the reverse, a saved give-away zeroed and stepped away from stays zeroed. **The rest**: a refused press marked the night dirty (it no longer does — the buckets are compared before and after); nori's refusal said *"Regular boxes"* and *"cheese"* (it says *"Sold with GCash"* and *"given away"* now); the head could read *"1.5 units"* (only whole figures are summed); the paper review printed *"Cheese not read"* beside a card showing the GCash-cheese count (it follows the card's blank rule now); the vision prompt never said `gcash` excludes the cheese boxes (one line, in the separately deployed `Vision.gs`); SETUP.md's daily flow still said *"of those"*. One finding was refuted and stands as written: the two nights' arithmetic is a consequence of the remap, not proof of the owner's reading — the Total cannot tell the readings apart, which is exactly why the code comment now says so and why he is asked to check the paper first.

**24 mutants, all red**, run on four rsync'd scratch copies with the suite in each: the old move on every branch of `uiStep` and `uiTyped` put back one at a time (G including GC, the of-those reclassification, the regular-GCash bump, the rewritten `gcash` on a typed cheese figure, the uncapped GC, the room that forgets the GCash-cheese boxes or the given-away units), dirty-on-refusal, the plural-only head, the fraction in the head, `freeHeld` true on an empty night, the draft dropping `free`, the card skipping nori, `freeOpen` never set, the head not refreshed, the card rendered after the GCash one, the save not opening it, the "Of those" label, the box-only toast on a simple sku, and the review's Cheese figure falling back to the reader's `cheese` alone. One anchor matched twice on the first pass — `custom: raw1(r.custom), free: raw1(r.free)` is byte-identical in `sanitizeDrafts` and `stashBentaDraft` — so that mutant was re-run anchored on the comment above it: killed by the draft test, as it should be. An anchor that matches twice is not a result; the rule holds for mutants as for edits.

Suites: **210** (`run-tests.js`) + **298** (`contract.test.js`) + **12 phone screens** (`smoke/phone.js`).

### v2.25.0 — the conversion looks at the cutoff, not the day

Owner: *"cash to gcash vice versa, looks at the overall total not just the day."* Two server tests and three phone tests new, two pins moved — and a review that found the bound could be hollowed out from underneath.

**The bound moved from the day to the cutoff so far, and the tests say which nights count.** The server test saves two earlier nights in the same cutoff (₱250 cash; ₱150 cash + ₱100 GCash), one in the *previous* cutoff and one *after* the night in question, then converts on the 30th: ₱711 refused naming *the cash this cutoff has taken in so far (710 up to July 30)* — the day's 310 plus 250 plus 150, not the previous cutoff's ₱500 and not the 31st's — ₱710 accepted with the day's own Cash at **−400** and the Total unmoved, the same ₱710 accepted again on a re-save (the night's own stored row, now negative, is excluded by date, or the room would have shrunk to 310), ₱711 still refused on that re-save, and the GCash direction mirrored at 190. The earlier nights' rows are read back untouched. The phone test builds the same shape through the real server and bootstrap, then proves preview, refusal and sheet agree: ₱800 on a ₱500 night previews as GCash 800 / Cash −300 and saves to the same figures; ₱901 is refused on the phone in the server's sentence, byte for byte, and the preview clamps at 900 so the receipt never shows a split the sheet will refuse; a GCash cash-out of ₱100 on a night with no GCash is allowed because an earlier night's ₱100 is in the balance; later nights and the previous cutoff do not count; a re-save of the converted night still has its full room. A second phone test pins the window rule: with the cutoff starting before `window_start` and a server configured, the phone neither clamps nor refuses (the server decides); with the window reaching the cutoff's start, or with no server at all, it judges by what it holds. The receipt's *"more than tonight's cash; the rest came out of earlier nights' cash this cutoff"* is render code: a source pin, both directions.

**Two pins moved, deliberately.** The cash-floor test's sentences now read *"the cash this cutoff has taken in so far (310 up to July 30)"* and *"the GCash this cutoff has taken in so far (90 up to July 30)"* — the figures are unchanged because that night is the only one in its cutoff, which is exactly the case where the new bound equals the old. The v2.7.3 both-ways test's regexes moved the same way.

**The review found the hole in the bound before it shipped.** Four lenses, two adversarial verifiers per finding, twenty agents, thirteen verdicts confirmed. The one that mattered: the bound was checked only on the night that *converted*, so re-saving an **earlier** night smaller — or closing it — after a later night had converted against its cash was never re-checked. The cutoff's Cash went below zero with no refusal, and the note printed *"Cash - -200"*. Reproduced by three verifiers independently on the real server. The fix restates the rule as an invariant that survives edits to other nights — *for every converting night on or after the one being saved, the cutoff's cash (or GCash) up to that night must not be below zero* — checked on **every** save on both sides, a closed-day save included, with a sentence that names the later night: *"Saving this would leave the cutoff short: the 350 converted to GCash on July 30 would be more than the cash taken in up to that day (250). Lower that conversion first, or check these counts."* The server test walks it: an earlier night of ₱250, a later night converting ₱350 on ₱100 of its own; the earlier night re-saved at 3 boxes refused naming the 30th and its ₱250, before any write; closed, refused at ₱100; one box short still short; exactly enough fine; a new earlier night only adds; a night *after* the converting one free to shrink or close, and its cash no help to the 30th; and the GCash direction mirrored on a fresh sheet. The phone test drives the same shape through the real server and proves the refusal byte for byte, on the GCash card for an open day and on the date slot for a closed one. Three more findings fixed alongside: a refusal in the *wrong direction's* sentence when the cutoff's cash was negative and a GCash cash-out was typed (the sign now guards each side); the phone counting a night the **server had refused** (it sits in `state.days` for the red card, but the sheet does not hold its cash — `dateNotInSheet` leaves it out); and a phone with a server configured but **never synced** judging a period it could not see (no `window_start` yet now means "the server decides", like a window that starts too late). The sheet audit gained rule 6b for the state only a hand edit can now produce, pinned in the same server test by editing a cell and asking. The wording moved too: the sentences say what the cutoff has *taken in*, not what is *in the tin*, because Mama's share and tin-paid expenses are not subtracted — a verifier caught the hint promising a figure the code does not compute. One confirmed finding stands as documented rather than fixed: offline, a backfilled earlier night can reach the server *after* a later night that leaned on it, and that night lands on the red card until it is re-saved (SPEC.md, v2.25.0).

**20 mutants, all red** — eight on the server, twelve on the phone, each side's room function broken the same way in turn: the night's own row counted on a re-save, later nights counted, the previous cutoff counted, the GCash side summing cash, the bound back to the day's (both directions), no bound at all, the sentence naming the period start instead of the day, the preview clamping at the day's cash, the window rule dropped or inverted ("any server means no judgement" — the mutant the added window assertion exists for), `computeDay` no longer reporting the day's two sides, and the receipt never saying where the rest came from. Every anchor unique first time.

**Then 18 more on the invariant, all red**: later converting nights never re-checked (both sides); the running sum counting nights *after* the converting one — killed only because the server test gained a night on the 31st and a re-save of the 28th that its cash must not rescue; the two sentences swapped; the audit rule silenced; a closed re-save skipping the check (both sides); the previous cutoff's nights counted; the check fed the night's figures *before* the conversion; the GCash cash-out judged by cash; a refused night counted again; a never-synced phone judging anyway; the phone's check reading the conversion clamped away off `computeDay` instead of by hand; the sentence naming the period start; the refusal running only when tonight converts; the receipt note's wording regressed to "the tin"; `conversionRoom` counting nights after the day.

Suites: **212** (`run-tests.js`) + **301** (`contract.test.js`) + **12 phone screens** (`smoke/phone.js`).
