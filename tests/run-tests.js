...
test('invalid token rejected; doGet ping needs no token', () => {
  ...
  assert.strictEqual(g.data.version, '2.6.0', 'VERSION was not bumped for this release');
  assert.strictEqual(post(ctx, { token, action: 'ping', payload: {} }).data.version, '2.6.0');
});
