...
test('invalid token rejected; doGet ping needs no token', () => {
  ...
  assert.strictEqual(g.data.version, '2.13.0', 'VERSION was not bumped for this release');
  assert.strictEqual(post(ctx, { token, action: 'ping', payload: {} }).data.version, '2.13.0');
});
...
test('an unknown action is refused by name, and doGet answers without a token', () => {
  ...
  const g = JSON.parse(ctx.doGet({}).getContent());
  assert.deepStrictEqual(g, { ok: true, data: { name: 'octogo-vision', version: '2.13.0' } });
});
...
test('ping proves the setup WITHOUT spending a unit of quota — even with no key yet', () => {
  ...
  assert.strictEqual(r.ok, true, r.error);
  assert.strictEqual(r.data.version, '2.13.0', 'the vision project ships with the release it belongs to');
