...
- `cutoff` payload `{start, end, dryRun}` → computes all note figures from DailyLog + Expenses + CutoffInputs in period; builds note_text in the exact format; if !dryRun upserts into Cutoffs by (start, end). Returns `{figures:{start, end, total, cash, gcash, mama, split, per_partner, supplies, octopus, salary, other, electric, supplies_minor, supplies_used, supplies_used_unpriced, backlog_in_minor, excluded, excluded_lines, remaining}, note_text}`. **`supplies` = Σ Expenses(category=Supplies) alone**, `octopus` and `other` likewise; **`supplies_minor` = supplies + octopus + other** (the one line the note prints, v2.20.0); **`supplies_used`** = Σ over StockUsage rows in period of `qty × (row.unit_cost if set, else the product's current unit_cost)`, and `supplies_used_unpriced` counts products with any row nobody could price (v2.22.0); `backlog_in_minor` = Σ Backlog payments, reported so a screen can warn about the overlap with `supplies_used`, never subtracted; `split` is the entered amount for the period else Settings `split_default`, `per_partner = split / 2`; `salary` = Σ each day's salary over non-closed days (a blank counts at the current `daily_salary`); **`remaining = total − mama − split − supplies_minor − supplies_used − salary − electric`** and may be negative. `excluded` / `excluded_lines` are display only and in no figure.
